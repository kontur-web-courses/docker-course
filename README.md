...
Останови контейнер, если он был запущен. Теперь выполни команду `docker run -it <id_образа> /bin/bash`. Затем установи `ping` с помощью `apt update && apt install iputils-ping -y`. После установки пропингуй любой адрес.

### 2.2. Присоединись к запущенному контейнеру
Во втором инстансе терминала напиши `docker ps`, чтобы посмотреть список запущенных контейнеров. Найди свой, затем выполни команду `docker attach <container_id>`. Проверь, что команда `ping` доступна.
